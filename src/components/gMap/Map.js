...
const Map = ({ paths, stops }) => {
  ...
  const velocity = 20; // 100km per hour
  let initialDate;
  let interval = null;
  const icon1 = {
  ...
  const getDistance = () => {
    ...
    const differenceInTime = (new Date() - initialDate) / 400; // pass to seconds
    return differenceInTime * velocity; // d = v*t -- thanks Newton!
  };
...
console.log(latLong2)
  ...
  const startSimulation = useCallback(
    () => {
      ...
      interval = window.setInterval(moveObject, 2000);
    },
